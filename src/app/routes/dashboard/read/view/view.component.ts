...
export class ViewComponent implements OnInit {
  ...
  getDocInfo(): void {
    ...
    this.readService.getDocInfo(params).subscribe(
  ...
  getQuestions(labelQuestionObj?: LabelQuestionInfoParams): void {
    ...
    this.readService.labelQuestion(params).subscribe(
      (value: ResponseParams) => {
        if (!value.code) {
  ...
  deleteQuestion(question: LabelQuestionInfoQuestionsParams, labelQuestion?: LabelQuestionInfoParams): void {
    ...
    this.readService.deleteLabelQuestion(params).subscribe(
      (value: ResponseParams) => {
        if (!value.code) {
   ...
   * @param question LabelQuestionInfoParams
   */
  deleteQuestionAndAnswer(questionAndAnswer: LabelQuestionInfoParams): void {
    this.modalService.confirm({
      nzTitle: `你确定要删除标注及其问题吗?`,
      nzOkText: '确定',
      nzOkType: 'danger',
      nzOnOk: () => this.delete(questionAndAnswer),
      nzCancelText: '取消',
    });
  }
  ...
  delete(question: LabelQuestionInfoParams): void {
    ...
    this.readService.deleteLabel(params).subscribe(
      (value: ResponseParams) => {
        if (!value.code) {
          this.msg.success('删除成功');
          this.getQuestions();
